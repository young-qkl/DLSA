...
class MultiModalSwinTransformer(nn.Module):
    ...
    def forward(self, x, l, l_mask, lang_feat=None):
        """Forward function."""
        x = self.patch_embed(x)
        if lang_feat is None:
            lang_feat = l[:, :, 0] if l.dim() == 3 else l[:, 0, :]  # B, 768
        Wh, Ww = x.size(2), x.size(3)
        if self.ape:
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, H, W, x, Wh, Ww = layer(x, Wh, Ww, l, l_mask, lang_feat=lang_feat, stage_id=i)
...
class MMBasicLayer(nn.Module):
    def __init__(self,
        ...
        self.sgAMC = SgAMC(in_channels=dim, out_channels=dim, mode='language')
        self.vs_fusion_alpha = nn.Parameter(torch.tensor(0.5))
        # dynamic alpha fc
        self.alpha_fc = nn.Linear(768, 1)  # 假设 lang_feat dim = 768

    def forward(self, x, H, W, l, l_mask, lang_feat=None, stage_id=0):
        """ Forward function.
        ...
        v_residual = self.visual_residual(x, lang_feat=lang_feat, stage_id=stage_id)
        s_residual = self.sgAMC(x, lang_feat=lang_feat, stage_id=stage_id)
        if s_residual.dim() == 4:
            bs, dim, h, w = s_residual.shape
            s_residual = s_residual.view(bs, dim, -1).permute(0, 2, 1)

        # compute dynamic alpha
        alpha = torch.sigmoid(self.alpha_fc(lang_feat)).view(-1, 1, 1)  # (B,1,1)

        vs_gate = alpha * torch.sigmoid(v_residual) + \
                  (1 - alpha) * torch.sigmoid(s_residual)

        # PWAM fusion
        x_residual = self.fusion(x, l, l_mask)
        # apply a gate on the residual
        x = x + (self.res_gate(x_residual) * x_residual) + vs_gate
        # if not self.training:
        #     alpha = torch.sigmoid(self.vs_fusion_alpha).item()
        #     v_norm = v_residual.norm(dim=-1).mean().item()
        #     s_norm = s_residual.norm(dim=-1).mean().item()
        #     print(f"[TEST] vs_fusion_alpha: {alpha:.4f}, v_norm: {v_norm:.4f}, s_norm: {s_norm:.4f}")
