...
BASE_OUTPUT_DIR="${BASE_OUTPUT_DIR:-${ROOT}/checkpoints/base_rrsisd}"
DLSA_OUTPUT_DIR="${DLSA_OUTPUT_DIR:-${ROOT}/checkpoints/dlsa_rrsisd}"
BASE_CHECKPOINT="${BASE_CHECKPOINT:-${BASE_OUTPUT_DIR}/model_best_RMSIN_RRSISD.pth}"
DLSA_CHECKPOINT="${DLSA_CHECKPOINT:-${DLSA_OUTPUT_DIR}/model_best_DLSA_RRSISD.pth}"

run_base() {
    DATA_ROOT="${DATA_ROOT}" OUTPUT_DIR="${BASE_OUTPUT_DIR}" \
        bash "${ROOT}/scripts/train_rrsisd_base.sh"
}

run_dlsa() {
    [[ -f "${BASE_CHECKPOINT}" ]] || { echo "Missing base checkpoint: ${BASE_CHECKPOINT}" >&2; exit 1; }
    DATA_ROOT="${DATA_ROOT}" OUTPUT_DIR="${DLSA_OUTPUT_DIR}" \
        INIT_CHECKPOINT="${BASE_CHECKPOINT}" bash "${ROOT}/scripts/train_rrsisd.sh"
}
...
run_test() {
    ...
    for split in val test; do
        DATA_ROOT="${DATA_ROOT}" CHECKPOINT="${DLSA_CHECKPOINT}" SPLIT="${split}" \
            bash "${ROOT}/scripts/test_rrsisd.sh" | tee "${DLSA_OUTPUT_DIR}/${split}.log"
    done
}
